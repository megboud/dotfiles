...
set mouse=n
set ruler
set runtimepath=~/vimfiles,C:\\Program\ Files\ (x86)\\Vim/vimfiles,C:\\Program\ Files\ (x86)\\Vim\\vim74,C:\\Program\ Files\ (x86)\\Vim/vimfiles/after,~/vimfiles/after,~/.vim,~\\.vim\\bundle\\ctrlp.vim,~\\.vim\\bundle\\molokai,~\\.vim\\bundle\\nerdtree,~\\.vim\\bundle\\syntastic,~\\.vim\\bundle\\vim-airline,~\\.vim\\bundle\\vim-orgmode,~\\.vim\\bundle\\vim-speeddating
set scrolloff=14
set showmatch
set smartcase
set textwidth=79
...
set shortmess=aoO
badd +1 Misc\Todo.org
badd +1 Misc\Log.org
badd +1 Misc\Research\ Log.org
silent! argdel *
edit Misc\Todo.org
...
setlocal statusline=%{airline#check_mode()}%#airline_a#%(\ %{get(w:,\ \"airline_current_mode\",\ \"\")}%{airline#extensions#paste#get_mark()}\ %)%#airline_a_to_airline_b#>%#airline_b#%#airline_b_to_airline_c#>%#airline_c#%<%(\ %f%m\ %)%#airline_c#\ %#airline_file#%{airline#extensions#readonly#get_mark()}\ %=%#airline_c_to_airline_x#<%#airline_x#%(\ %{strlen(&filetype)>0?&filetype:''}\ %)%#airline_x_to_airline_y#<%#airline_y#%(\ %{strlen(&fenc)>0?&fenc:''}%{strlen(&ff)>0?'['.&ff.']':''}\ %)%#airline_y_to_airline_z#<%#airline_z#%(\ %3p%%\ :%3l:%3c\ %)%(%#airline_z_to_airline_warningmsg#<%#airline_warningmsg#\ %{airline#extensions#whitespace#check()}%{SyntasticStatuslineFlag()}%)
